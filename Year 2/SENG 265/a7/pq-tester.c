...
Patient *tokenize_line(char *line) {
    ...
    */
    ...
    /*Grabs the first element of the line*/
    char *token = strtok(strtok(line, "\n"), ",");
    
    /*Compare*/
    if (strcmp(token, "enqueue") == 0) {
    	
    	/*Return a new Patient element*/
    ...
    /*If we have a command we don't recognize, return NULL*/
    return NULL;
}
...
Patient *read_lines(Patient *list) {
    ...
    */
    ...
    while (fgets(buf, MAX_LINE_LEN, stdin) != NULL) {
		...
		/*If we have a new Patient, add it, otherwise remove the front*/
		if (new != NULL) {
...
void deallocate_memory(Patient *list) {
    ...
    */
    ...
    /*While list is not null, store the next element, then free the first element*/
    while (cur != NULL) {
		 next = cur->next;
		 free(cur->name);
		 free(cur);
