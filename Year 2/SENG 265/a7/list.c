...
Patient *add_with_priority(Patient *list, Patient *new) {
    /* TODO: You have to implement this method to add patients with the
     ...
     curr = list;
     temp = malloc(sizeof(Patient));
     
     /*Bubble down the element to the correct spot*/
     while (curr->next != NULL && curr->priority >= curr->next->priority) {  
				memcpy(temp, curr, sizeof(Patient));
				
				/*Swap the elements, effectively swapping the elements*/
			   curr->name = curr->next->name;
			   curr->birth_year = curr->next->birth_year;
			   curr->priority = curr->next->priority;
			   curr->next->name = temp->name;
			   curr->next->birth_year = temp->birth_year;
