...
; Date of completed work: 11/23/2023
;
; **********************************
...
;Definining registers for timer1
.def DATAH=r25
.def DATAL=r24
.def BOUNDARY_H=r1
.def BOUNDARY_L=r0
...
start:
	...
	;load the boundry of 900 to detect if a button is being pressed
	ldi r16, low(900);
	mov BOUNDARY_L, r16
	ldi r16, high(900)
	mov BOUNDARY_H, r16
...
timer1:
	;Save the status registers and all registers we use
	push r16
	lds r16, SREG
	push r16
	lds	r16, ADCSRA_BTN	
...
wait:	lds r16, ADCSRA_BTN
		...
		;update BUTTON_IS_PRESSED
		clr r16
		sts BUTTON_IS_PRESSED, r16
		;stays 0 if branch not set
		cp DATAL, BOUNDARY_L
		cpc DATAH, BOUNDARY_H
		brsh skip
		;changed to one and upload to memory
		ldi r16, 1
		sts BUTTON_IS_PRESSED, r16
	skip:
		;fix the stack
...
timer3:
	...
	update_lcd:
		...
		;determines whether or not we need star or dash
		cpi r16, 1
		breq set_star
		rjmp set_dash

		;Visualized whether a button was pressed or not
		;Add dash if it is not set
		set_dash:
			ldi r16, '-'
			push r16
			rcall lcd_putchar
