...
;find the max length of the avail. charset
find_length:
	clr r17
...
;initialized the top line to be all clear
init_top_line:
	push r16
	push r17
	push r18
...
timer3:
	...
	;sets the position 0, 0 to the contents of TOP_LINE_CONTENT, which will be the first element of that
	update_top_line:
		in ZL, SPL
		in ZH, SPH
		ldi r16, 0
		...
		;grab the element at TOP_LINE_CONTENT
		ldi r31, high(TOP_LINE_CONTENT)
		ldi r30, low(TOP_LINE_CONTENT)

		;store this value for lcd_putchar
		ld r17, Z
		push r16 ;row
		push r18 ;column
		rcall lcd_gotoxy
...
timer4:
	;store the SREG and all registers we modify
	push r16
	lds r16, SREG
	push r16
	push r17
		...
		;define these pseudo registers for later
		ldi r31, high(AVAILABLE_CHARSET<<1)
		ldi r30, low(AVAILABLE_CHARSET<<1)	
		...
		;only happen if a button is being pressed
		lds r16, BUTTON_IS_PRESSED
		cpi r16, 1
		brne skip_skip4

		;compare the button being pressed
		lds r18, LAST_BUTTON_PRESSED
		cpi r18, 'U'
		...
		;modifies the CURRENT_CHARSET_INDEX by inc, limiting the max to be CHARSET_LEN
		up_button_func4:
			ld r17, x
			lds r16, CHARSET_LEN
			cp r17, r16
		...
		;modifies the CURRENT_CHARSET_INDEX by dec, limiting the min to be index 0
		down_button_func4:
			ld r17, x
			cpi r17, 0
			breq skip_4
		...
		;Grab the position of the CURRENT_CHARSET_INDEX in AVAIL. Charset, done by adding to the address of it. Store in TOP_LINE_CONTENT at the first byte
		update_line:
			clr r19
			ld r16, X
			add r30, r16
	...
	;fix all the stored values to fix stack
	skip_4:
		pop r19
		pop r18
		pop r17
...
.dseg
;used to store and keep the length of AVAILABLE_CHARSET
CHARSET_LEN: .byte 1
