...
start:
	...
	;Make sure the previous does not affect us somehow
	ldi r16, ' ' 
...
wait:	lds r16, ADCSRA_BTN
	...
	;uses all lows and highs of the max value of each button to tell which buttone was pressed
	...
	;functions for each button pressed
	...
	down_button_pressed:
		...
		sts LAST_BUTTON_PRESSED, r16
		rjmp skip
	;while functionally useless, we want to track to prevent errors
...
timer3:
	...
	update_lcd:
		...
		;set the visualizer that a button was pressed
		set_dash:
			ldi r16, '-'
			push r16
			rcall lcd_putchar
		...
		;also clear the bottom section given a new buttton pressed
		set_star:
			;select should only affect the star, so we want to skip the next
			;part of the code if LAST_BUTTON_PRESSED was select
			...
			ldi r16, ' '
			push r16
			;clear the 4 bottom right columns
			rcall lcd_putchar
			rcall lcd_putchar
			rcall lcd_putchar
			rcall lcd_putchar
			...
			;check and find the buttons pressed
			check_button_pressed:
				...
				;select will be missed and go here
				rjmp reset_timer_3


			;signify the lcd position based on the button pushed, these functions set the position for lcd_gotoxy
			right_button_sub:
				ldi r16, 1
				ldi r17, 3
				push r16 ;row
			...
			;visualizes the character using the register r17
			push_button_char:
				push r18
				rcall lcd_putchar
				pop r16
