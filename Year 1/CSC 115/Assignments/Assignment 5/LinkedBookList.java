public class LinkedBookList {
	...
	private int totalBooksRec(BookNode cur) {
		if (cur != null) {
			return 1 + totalBooksRec(cur.getNext());
		}
		else {
			return 0;
		}
	...
	private int totalPagesRec(BookNode cur) {
		if (cur != null) {
			return cur.getData().getPages() + totalPagesRec(cur.getNext());
		}
		else {
			return 0;
		}
	...
	private int countLongerThanRec(BookNode cur, Book sampleBook) {
		if (cur != null) {
			return ((cur.getData().getPages() > sampleBook.getPages())? 1 : 0) + countLongerThanRec(cur.getNext(), sampleBook);
		}
		else {
			return 0;
		}
	...
	private int pagesBeforeBookByAuthorRec(BookNode cur, String authorName) {
		if (cur != null) {
			return ((cur.getData().getAuthor().equals(authorName))? 0: (cur.getData().getPages() + pagesBeforeBookByAuthorRec(cur.getNext(), authorName)));
		}
		else {
			return 0;
		}
	...
	private Book longestBookRec(BookNode cur, Book longest) {
		if (cur == null) {
			...
			return longestBookRec(cur.getNext(), longest);
	...
	private int distanceBetweenBooksByAuthorRec(BookNode cur, String authorName, boolean firstBookFound) {
		...
		if (cur.getData().getAuthor() == authorName && !firstBookFound) {
			firstBookFound = true;
			return distanceBetweenBooksByAuthorRec(cur.getNext(), authorName, firstBookFound);
		}
		else if (cur.getData().getAuthor() != authorName && firstBookFound) {
			return 1 + distanceBetweenBooksByAuthorRec(cur.getNext(), authorName, firstBookFound);
		}
		else if (cur.getData().getAuthor() == authorName && firstBookFound){
			return 0;
		}
