public class Podcast {
	...
	public boolean isShorterThan(int duration) {
		return (this.duration < duration); // so it compiles
	}
	...
	public double averageFollowers() {
		...
		return sum/((hosts.length > 0)? hosts.length : 1);
	}
	...
	public boolean hasHost(Influencer alias) {
		boolean found = false;
		int index = 0;
		while (index < hosts.length && !found) {
			if (hosts[index].isNamed(alias.getName())) {
				found = true;
			}
			index ++;
		}
