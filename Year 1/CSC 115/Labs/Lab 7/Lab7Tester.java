...
public class Lab7Tester {
	...
	public static void testBasicStack() {
		...
		try {
		    s.makeEmpty();
			s.pop();
		    displayResults(false, "exception should be thrown");
		}  catch (StackEmptyException see) {
	...
	public static void testQueue() {
		...
		displayResults(q.peek() == 1, "makeEmpty + enqueue + dequeue + peek - queue with multiple elements");
			...
			System.out.println("Returned an Exception when it wasn't expected");
		}
		try {
			q.makeEmpty();
		    q.dequeue();
		...
		try {
			...
			q.dequeue();
			q.peek();
			displayResults(false, "Enqueue + Dequeue + Peek should throw an exception");
		}
		catch (QueueEmptyException e) {
			displayResults(true, "Should throw an Empty exception");
		}
